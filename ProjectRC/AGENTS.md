...
`$randomchat-autopilot`은 HANDOFF의 `자동 진행 목록`에서 최대 세 개의 작은 작업만 연속으로 처리한다. 각 작업마다 구현, 관련 테스트 또는 APK 빌드, HANDOFF 갱신을 끝낸다. 실기기 시험, 외부 계정, 데이터 삭제, 정책 결정이 필요하면 멈추고 이유를 기록한다.
